...
class Joy_caption_alpha_batches:
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                "JoyPipeline_alpha": ("JoyPipeline_alpha",),
                "img_root": ("STRING", {"multiline": True, "default": ""},),
                "save_root": ("STRING", {"multiline": True, "default": ""},),
                "trigger": ("STRING", {"multiline": False, "default": "trigger"},),
                "prompt": ("STRING", {"multiline": True, "default": "A descriptive caption for this image"},),
                "format": (["png", "jpg"],),
                "max_new_tokens": ("INT", {"default": 1024, "min": 10, "max": 4096, "step": 1}),
    ...
    def gen(self, JoyPipeline_alpha, img_root, save_root, trigger, prompt, format, max_new_tokens, temperature, cache,
            low_vram, seed):

        for batch_name in os.listdir(img_root):
            img_dir = os.path.join(img_root, batch_name)
            save_dir = os.path.join(save_root, batch_name)
            ...
            for image_path in dir_files:
                ...
                img_save_path = os.path.join(save_dir, img_file_name)
                # input_image.save(img_save_path)
                os.system(f"ln -s {image_path} {img_save_path}")
